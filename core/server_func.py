...
class Server():
	...
	def broadcast_msg(self, data: bytes, nickname: str):
		data = b"[" + nickname + b"]: " + data
		self.last_msg = data.decode()
		for index, client in enumerate(self.client_dict["CLIENT_OBJ"]):
			self.send_encrypt(client_obj=client, data=data, pubkey=self.client_dict["CLIENT_KEYS"][index])
	...
	def __accept_thread(self, socket_obj: socket.socket) -> None:
		while True:
			...
			if client_nick.decode().lower() == "server":
				client_nick = b"server_"
